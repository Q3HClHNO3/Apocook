(function () {
  // Unified data layer: data/ingredients.json, data/recipes.json, data/contributors.json, data/cityRoutes.json, data/flights.json, data/destinations.json.
  ...
  function normalizeContributors(rawContributors) {
    if (Array.isArray(rawContributors)) return rawContributors;
    if (rawContributors && Array.isArray(rawContributors.contributors)) return rawContributors.contributors;
    return [];
  }
  ...
  async function initDatabase() {
    const [ingredients, recipes, contributors, recipeCategories, cityRoutes, flights, destinations] = await Promise.all([
      loadJson(new URL("ingredients.json", DATA_BASE_URL).href, {}),
      loadJson(new URL("recipes.json", DATA_BASE_URL).href, []),
      loadJson(new URL("contributors.json", DATA_BASE_URL).href, []),
    ...
    window.APP_DATA.contributors = normalizeContributors(contributors);
  ...
  window.getContributorData = function (id) {
    const contributorId = normalizeId(id);
    if (!contributorId) return null;
    return (window.APP_DATA.contributors || []).find((contributor) => normalizeId(contributor?.id) === contributorId) || null;
  };

  window.getAllContributors = function () {
    return window.APP_DATA.contributors || [];
  };

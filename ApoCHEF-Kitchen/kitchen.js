      ...
      function getRealFridgeReminder() {
        ...
        return recipeName
          ? `乖孙，现实里的【${ingredientName}】再不吃就要黄啦！今天不如做个【${recipeName}】？`
          : `乖孙，现实里的【${ingredientName}】快要不新鲜啦，今晚记得先吃它。`;
      }
      ...
      function rebuildShoppingListFromTodayMenus() {
        ...
        const realFridgeSet = new Set(realFridge()?.getRealFridgeIngredientIds?.() || []);
        ...
        menus.forEach((menu) => {
          ...
          getRecipeRequiredIngredientIds(recipe).forEach((ingredientId) => {
            ...
            const existing = targetMap.get(ingredientId) || {
              ingredientId,
              status: luggageSet.has(ingredientId) || realFridgeSet.has(ingredientId) ? "collected" : "needed",
              recipeIds: [],
              recipeNames: [],
              createdAt: now,
              updatedAt: now
            ...
            existing.status = luggageSet.has(ingredientId) || realFridgeSet.has(ingredientId) ? "collected" : "needed";
      ...
      function renderTodayMenuNote() {
        ...
        if (shoppingToRealFridgeBtn) {
          shoppingToRealFridgeBtn.hidden = neededCount === 0;
          shoppingToRealFridgeBtn.textContent = neededCount
            ? `买回来了，放入雪柜（${neededCount}）`
            : "买回来了，放入雪柜";
        }
      ...
      function moveShoppingListToRealFridge() {
        ...
        const targets = shoppingList.filter((item) => item?.ingredientId && item.status !== "collected");

        if (!targets.length) {
          flashSceneToast("采购清单暂时没有需要归仓的食材。");
          return;
        }
        ...
        const now = Date.now();
        writeShoppingList(shoppingList.map((item) => (
          item?.ingredientId && targets.some((target) => target.ingredientId === item.ingredientId)
            ? { ...item, status: "collected", updatedAt: now }
            : item
        )));

        refreshRealFridgeUi(`买回来的 ${targets.length} 样食材已经放入现实雪柜。`);
      }
      ...
      function deductRealFridgeForRecipe(recipe) {
        ...
        renderTodayMenuNote();
        window.setTimeout(() => {
          flashSceneToast("这道菜用掉了现实雪柜里的部分食材。");
        }, 260);
      }
      ...
      function renderRecipeBook() {
        ...
        const renderRecipeCard = (recipe) => {
            ...
            const requiredIngredientIds = getRecipeRequiredIngredientIds(recipe);
            const ownedCount = requiredIngredientIds.filter(isIngredientOwnedForRecipe).length;
            const missingCount = requiredIngredientIds.length - ownedCount;
            ...
            return `
              <button class="recipe-card ${inTodayMenu ? "is-selected" : ""}" type="button" data-recipe-id="${escapeHtml(recipe.id)}">
                ...
                <span class="recipe-card-missing ${missingCount ? "" : "is-ready"}">
                  ${missingCount ? `已有 ${ownedCount}/${requiredIngredientIds.length} 样，还差 ${missingCount} 样` : `食材齐啦 · ${requiredIngredientIds.length} 样`}
                </span>
      ...
      function renderIngredientButton(ingredient) {
        ...
        const hasRealFridgeItem = freshness !== "missing";
        ...
        return `
          <div class="fridge-item-card">
            ...
            <div class="real-fridge-actions" aria-label="${escapeHtml(ingredientData.name)}现实雪柜操作">
              ...
              ${hasRealFridgeItem ? `<button class="real-fridge-action-btn" type="button" data-real-fridge-action="used" data-ingredient-id="${escapeHtml(ingredient.id)}">吃完了</button>` : ""}
              ${hasRealFridgeItem ? `<button class="real-fridge-action-btn" type="button" data-real-fridge-action="remove" data-ingredient-id="${escapeHtml(ingredient.id)}">移除</button>` : ""}
            </div>
            ${removeButton}
          </div>
        `;
      ...
      function bindFridgeDrawers() {
        ...
        document.querySelectorAll(".real-fridge-action-btn").forEach((button) => {
          button.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            ...
            if (action === "bought") {
              ...
              refreshRealFridgeUi(`${ingredientName} 已记入现实雪柜。`);
            } else if (action === "used") {
              realFridge()?.markUsed?.(ingredientId, 1);
              refreshRealFridgeUi(`${ingredientName} 已从现实雪柜扣减。`);
            } else if (action === "remove") {
              realFridge()?.removeFridgeItem?.(ingredientId);
              refreshRealFridgeUi(`${ingredientName} 已从现实雪柜移除。`);
            }
          });
        });
      }
      ...
      shoppingToRealFridgeBtn?.addEventListener("click", moveShoppingListToRealFridge);

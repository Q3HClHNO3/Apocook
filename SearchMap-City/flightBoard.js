    ...
    function renderShoppingNote() {
      ...
      const shoppingNoteTitle = shoppingNoteToggle?.querySelector("strong");

      if (!summary.total) {
        if (shoppingNoteTitle) shoppingNoteTitle.textContent = "阿婆采购清单";
        shoppingNoteText.textContent = "今天还没有采购任务。先回厨房点几道菜吧。";
        if (shoppingMissingList) shoppingMissingList.innerHTML = "";
        return;
      }

      if (shoppingNoteTitle) shoppingNoteTitle.textContent = "阿婆采购清单 · 点击查看缺什么";
      shoppingNoteText.textContent = summary.needed
        ? `还差 ${summary.needed} 样，点击查看清单`
        : "食材已找齐，可以打包回厨房";
    ...
    function openLuggage() {
      renderShoppingNote();
      renderLuggageList();
      const shouldShowShoppingList = readShoppingList().length > 0;
      shoppingNoteToggle?.setAttribute("aria-expanded", String(shouldShowShoppingList));
      if (shoppingNoteDetail) shoppingNoteDetail.hidden = !shouldShowShoppingList;

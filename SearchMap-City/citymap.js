      ...
      function getShoppingIngredientIdsFromUrl() {
        const value = new URLSearchParams(window.location.search).get("shopping") || "";
        return new Set(value.split(",").map((id) => id.trim()).filter(Boolean));
      }

      function getFocusIngredientIdFromUrl() {
      ...
      function getShoppingTargetSet() {
        return window.AppInventory.getShoppingTargetSet();
      }

      function syncShoppingListStatuses() {
        return window.AppInventory.syncShoppingListStatuses();
      }

      function renderShoppingNote() {
        if (!shoppingNoteText) return;
        const summary = window.AppInventory.getShoppingListSummary();
        ...
        shoppingNoteText.textContent = `已找到 ${summary.collected}/${summary.total} 样食材，还差 ${summary.needed} 样。`;

        if (!shoppingMissingList) return;
        shoppingMissingList.innerHTML = [
          summary.neededItems.length ? `<li class="shopping-detail-heading">还差</li>` : "",
          ...
          summary.collectedItems.length ? `<li class="shopping-detail-heading">已找到</li>` : "",
          ...summary.collectedItems.map((item) => {
            const ingredient = window.getIngredientData(item.ingredientId);
            return `
      ...
      function removeLuggageItem(index) {
        const currentLuggage = readLuggage();
        if (index < 0 || index >= currentLuggage.length) return;

        const nextLuggage = window.AppInventory.removeLuggageItemAt(index);
        renderLuggageList();
        updateLuggageBadge(nextLuggage.length);
        updateEatAllButton(nextLuggage.length);
        renderShoppingNote();
        refreshShoppingTargetMarkers();
      }
      ...
      function eatAllLuggageItems() {
        ...
        window.AppInventory.clearLuggage();
        renderLuggageList();
      ...
      function makeIngredientIcon(hotspot) {
        const luggageSet = window.AppInventory.getOwnedIngredientIds();
        const shoppingTargets = getShoppingTargetSet();
        const urlShoppingTargets = getShoppingIngredientIdsFromUrl();
        const focusId = getFocusIngredientIdFromUrl();
        const isTarget = hotspot.kind === "ingredient"
          && (shoppingTargets.has(hotspot.ingredientId) || urlShoppingTargets.has(hotspot.ingredientId))
          && !luggageSet.has(hotspot.ingredientId);
        const isFocusTarget = isTarget && focusId === hotspot.ingredientId;
        ...
        return L.divIcon({
          ...
          html: `
            <div
              class="ingredient-marker ${isTarget ? "is-shopping-target-marker" : ""} ${isFocusTarget ? "is-focus-target-marker" : ""}"
              role="button"
              tabindex="0"
      ...
      function setBasemap(type = "story") {
        if (!map) return;
        const config = BASEMAPS[type] || BASEMAPS.story;
        const activeType = BASEMAPS[type] ? type : "story";

        if (activeBasemapLayer) {
          map.removeLayer(activeBasemapLayer);
        }

        activeBasemapLayer = L.tileLayer(config.url, config.options).addTo(map);
        document.body.dataset.mapStyle = activeType;

        document.querySelectorAll("[data-map-style]").forEach((button) => {
          button.classList.toggle("is-active", button.dataset.mapStyle === activeType);
        });
      }
      ...
      function bindBasemapSwitcher() {
        document.querySelectorAll("[data-map-style]").forEach((button) => {
          button.addEventListener("click", () => {
            setBasemap(button.dataset.mapStyle);
          });
        });
      }

      function updateFlightRouteVisibility() {
        if (!map || !flightRouteLayer) return;
        const isWorldMode = map.getZoom() <= 5;
        flightRouteLayer.eachLayer((line) => {
          line.setStyle({
            opacity: isWorldMode ? 0.42 : 0,
            weight: isWorldMode ? 2 : 0.1
          });
        });
      }
      ...
      function updateWorldMode() {
        if (!map) return;
        document.body.classList.toggle("is-world-map", map.getZoom() <= 5);
        updateFlightRouteVisibility();
      }

      function renderFlightRoutes(cities) {
        if (!map) return;
        ...
        const cityList = Object.values(cities || {}).filter((city) => Array.isArray(city.center));
        const guangzhou = cityList.find((city) => normalizeCityKey(city.cityId || city.displayName) === "guangzhou");

        if (!guangzhou) return;

        const lines = cityList
          .filter((city) => city !== guangzhou)
          .map((city) => L.polyline([guangzhou.center, city.center], {
            color: "#6ac7df",
            weight: 2,
            opacity: 0.42,
            dashArray: "6 10",
            className: "apocook-flight-route",
            interactive: false
          }));

        if (!lines.length) return;

        flightRouteLayer = L.layerGroup(lines).addTo(map);
        updateFlightRouteVisibility();
      }
      ...
      function refreshShoppingTargetMarkers() {
        ...
        markerByStopId.forEach((marker) => {
          ...
          const isTarget = ingredientId && (shoppingTargets.has(ingredientId) || urlShoppingTargets.has(ingredientId)) && !luggageSet.has(ingredientId);
          markerButton.classList.toggle("is-shopping-target-marker", isTarget);
          markerButton.classList.toggle("is-focus-target-marker", isTarget && ingredientId === focusId);
        });
      }
      ...
      function triggerShoppingFocusSequence(hotspots = []) {
        const focusId = getFocusIngredientIdFromUrl();
        if (!focusId || !map) return;
        const hotspot = findTraceHotspot(focusId, hotspots);
        if (!hotspot) {
          const ingredient = window.getIngredientData?.(focusId);
          showToast(`阿婆暂时没在这张地图上找到 ${ingredient?.name || focusId}。`);
          return;
        }

        map.flyTo([hotspot.lat, hotspot.lng], Math.max(map.getZoom(), 16), {
          animate: true,
          duration: 1.4,
          easeLinearity: 0.25
        });
        window.setTimeout(() => {
          const marker = markerByStopId.get(hotspot.stopId);
          const element = marker?.getElement()?.querySelector(".ingredient-marker");
          element?.focus?.({ preventScroll: true });
          showToast(`阿婆出发：先找 ${hotspot.name}。`);
        }, 1450);
      }
      ...
      function returnToKitchenFromTrace(ingredientId) {
        ...
        params.set("openFridge", "1");
        params.set("from", "trace-origin");
        params.set("returnTo", "fridge");
      ...
      function collectActiveIngredient() {
        ...
        const urlShoppingTargets = getShoppingIngredientIdsFromUrl();
        const wasShoppingTarget = getShoppingTargetSet().has(ingredientId) || urlShoppingTargets.has(ingredientId);
        if (!luggage.includes(ingredientId)) {
          luggage.push(ingredientId);
          writeLuggage(luggage);
        }
        syncShoppingListStatuses();
        const isShoppingHit = !wasAlreadyPacked && wasShoppingTarget;
      ...
      function returnToKitchenWithLuggage() {
        window.AppInventory.returnLuggageToPantry();
      ...
      function bindEvents() {
        ...
        map.on("moveend", updateWorldMode);
        map.on("move zoom", () => {
      ...
      async function initCityMap() {
        try {
          ...
          } else if (getShoppingIngredientIdsFromUrl().size) {
            window.setTimeout(() => {
              triggerShoppingFocusSequence(hotspots);
            }, 900);
          }
        } catch (error) {
          console.error(error);
